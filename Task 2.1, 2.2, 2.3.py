...
eps_values = np.linspace(0.20, 0.00, 21)
reliabilities = (1 - eps_values) * 100  # Convert to percentages (80% to 100%)
...
for eps in eps_values:
    # Get the optimal in-sample bid using ALSO-X (your exact_p90_bid function)
    bid = exact_p90_bid(A_in, eps)
    bids_alsox.append(bid)
    
    # Test it on the out-of-sample data
    out_results = verify_bid(out_of_sample, bid)
    
    # Extract the expected reserve shortfall (average shortfall across all profiles)
    expected_shortfalls.append(out_results["avg_max_shortfall_kW"])
...
ax1.set_ylabel('Optimal Reserve Bid [kW] (In-Sample)', color=color)
ax1.plot(reliabilities, bids_alsox, marker='o', color=color, label='Reserve Bid')
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twinx()  
color = 'tab:red'
ax2.set_ylabel('Expected Reserve Shortfall [kW] (Out-of-Sample)', color=color)
ax2.plot(reliabilities, expected_shortfalls, marker='s', color=color, label='Expected Shortfall')
